Connecting to aws-0-eu-west-1.pooler.supabase.com 6543
[31mNotFound: ChildProcess.spawn (docker run --rm --network host --env PG_META_DB_URL=postgresql://sandbox_exec.ejtpncgssawecdzdrehz:8903c0b6-fb04-4b47-81d0-de247612e893@aws-0-eu-west-1.pooler.supabase.com:6543/postgres --env PG_CONN_TIMEOUT_SECS=15 --env PG_QUERY_TIMEOUT_SECS=15 --env PG_META_GENERATE_TYPES=typescript --env PG_META_GENERATE_TYPES_INCLUDED_SCHEMAS=public --env PG_META_GENERATE_TYPES_SWIFT_ACCESS_CONTROL=internal --env PG_META_GENERATE_TYPES_DETECT_ONE_TO_ONE_RELATIONSHIPS=true --env PG_META_DB_SSL_ROOT_CERT=-----BEGIN CERTIFICATE-----

Nzbc6lnA69urKffvcT07YwMsY/OmHzVa
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
 public.ecr.aws/supabase/postgres-meta:v0.96.1 node dist/server/server.js)[39m
Try rerunning the command with --debug to troubleshoot the error.
